...
class StandardRoIHeadAdaptive(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def forward_train(self, x, img_metas, proposal_list, gt_bboxes, gt_labels, gt_bboxes_ignore=None, gt_masks=None):
        ...
        if self.with_bbox or self.with_mask:
            ...
            for i in range(num_imgs):
                assign_result = self.bbox_assigner.assign(proposal_list[i], gt_bboxes[i], gt_bboxes_ignore[i],
                                                          gt_labels[i])
        ...
        if self.with_mask:
            mask_results = self._mask_forward_train(x, sampling_results, bbox_results['bbox_feats'], gt_masks,
                                                    img_metas)
    ...
    def _bbox_forward(self, x, rois):
        ...
        cls_score, bbox_pred, feat_rcnn_shared, feat_rcnn_cls, feat_rcnn_bbox = self.bbox_head(bbox_feats)

        bbox_results = dict(cls_score=cls_score,
                            bbox_pred=bbox_pred,
                            feat_roi=feat_roi,
                            feat_rcnn_shared=feat_rcnn_shared,
                            feat_rcnn_cls=feat_rcnn_cls,
                            feat_rcnn_bbox=feat_rcnn_bbox)
        return bbox_results
    ...
    async def async_simple_test(self, x, proposal_list, img_metas, proposals=None, rescale=False):
        ...
        if not self.with_mask:
            ...
            segm_results = await self.async_test_mask(x,
                                                      img_metas,
                                                      det_bboxes,
                                                      det_labels,
                                                      rescale=rescale,
    ...
    def mask_onnx_export(self, x, img_metas, det_bboxes, det_labels, **kwargs):
        ...
        batch_index = torch.arange(det_bboxes.size(0),
                                   device=det_bboxes.device).float().view(-1, 1,
                                                                          1).expand(det_bboxes.size(0),
                                                                                    det_bboxes.size(1), 1)
    ...
    def bbox_onnx_export(self, x, img_metas, proposals, rcnn_test_cfg, **kwargs):
        ...
        rois = proposals
        batch_index = torch.arange(rois.size(0),
                                   device=rois.device).float().view(-1, 1, 1).expand(rois.size(0), rois.size(1), 1)
