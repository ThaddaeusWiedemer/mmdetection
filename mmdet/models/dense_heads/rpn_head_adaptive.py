...
class RPNHeadAdaptive(AnchorHead):
    """RPN head that passes on more information.
    ...
    def __init__(self, in_channels, init_cfg=dict(type='Normal', layer='Conv2d', std=0.01), **kwargs):
        super(RPNHeadAdaptive, self).__init__(1, in_channels, init_cfg=init_cfg, **kwargs)
    ...
    def loss(self, cls_scores, bbox_preds, gt_bboxes, img_metas, gt_bboxes_ignore=None):
        ...
        """
        losses = super(RPNHeadAdaptive, self).loss(cls_scores,
                                           bbox_preds,
                                           gt_bboxes,
                                           None,
                                           img_metas,
    ...
    def get_bboxes(self, cls_scores, bbox_preds, img_metas, cfg=None, rescale=False, with_nms=True):
        ...
        for img_id in range(len(img_metas)):
            ...
            proposals = self._get_bboxes_single(cls_score_list, bbox_pred_list, mlvl_anchors, img_shape, scale_factor,
                                                cfg, rescale)
    ...
    def onnx_export(self, x, img_metas):
        ...
        for idx in range(len(cls_scores)):
            ...
            if nms_pre > 0:
                ...
                rpn_bbox_pred = rpn_bbox_pred.reshape(-1, 4)[transformed_inds, :].reshape(batch_size, -1, 4)
                anchors = anchors.reshape(-1, 4)[transformed_inds, :].reshape(batch_size, -1, 4)
            mlvl_scores.append(scores)
            mlvl_bbox_preds.append(rpn_bbox_pred)
            mlvl_valid_anchors.append(anchors)
        ...
        dets, _ = add_dummy_nms_for_onnx(batch_mlvl_proposals, batch_mlvl_scores, cfg.max_per_img,
                                         cfg.nms.iou_threshold, score_threshold, nms_pre, cfg.max_per_img)
        return dets
