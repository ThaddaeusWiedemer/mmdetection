...
class IterBasedRunnerAdaptive(BaseRunner):
    """Iteration-based Runner for domain adaptation.

    This runner train models iteration by iteration with a source and target
    dataset.
    """
    ...
    def train(self, data_loader_src, data_loader_tgt, **kwargs):
        ...
        self.data_loader_tg = data_loader_tgt
        # 1 epoch means we exhausted target domain data once
        self._epoch = data_loader_tgt.epoch
        data_batch = next(data_loader_src)
        data_batch_tgt = next(data_loader_tgt)
        # rename target keys
    ...
    def val(self, data_loader_src, data_loader_tgt, **kwargs):
        ...
        data_batch = next(data_loader_tgt)
    ...
    def run(self, data_loaders_src, data_loaders_tgt, workflow, max_iters=None, **kwargs):
        """Start running.
        ...
        iter_loaders_src = [IterLoader(x) for x in data_loaders_src]
        iter_loaders_tgt = [IterLoader(x) for x in data_loaders_tgt]
            ...
            for i, flow in enumerate(workflow):
                ...
                for _ in range(iters):
                    if mode == 'train' and self.iter >= self._max_iters:
                        break
                    iter_runner(iter_loaders_src[i], iter_loaders_tgt[i], **kwargs)
