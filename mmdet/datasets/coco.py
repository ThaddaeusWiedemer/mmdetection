...
class CocoDataset(CustomDataset):
    ...
    def _compute_lamr(self, coco_evalImgs, coco_eval, coco_paramsEval):
        """

        Args:
            coco_evalImgs (list[dict]): detections generated by pycocotools
                after cocoEval.evaluate() for every image, category, and
                area size in the form of a dictionary containing the fields:
            ...
            coco_eval: the dictionary generated by pycocotools after
                cocoEval.accumulate()
            coco_paramsEval: evaluation parameters used in pycocotools
        ...
        Returns:
            float, ndarray: LAMR value, mr@FPPI curve as ndarray[[fppi], [mr], [confidence]]
        """
        print('Calculating miss-rate@FPPI and LAMR...')
        p = coco_paramsEval
        ...
        # for LAMR we average over all classes, consider all object sizes,
        # and don't limit the number of detections
        miss_rate = np.mean(mr, axis=1)[:,0,0]
        confidence = np.mean(scores, axis=1)[:,0,0]
        
        # miss-rate and confidence can't be zero
        miss_rate[miss_rate==0] = np.nan
        confidence[confidence==0] = np.nan

        # compute LAMR at sample locations. If a certain FPPI threshold isn't reached
        # the minimum miss-rate achieved so far is used
    ...
    def evaluate(self,
        ...
        for metric in metrics:
            ...
            if metric == 'proposal':
                ...
                if metric_items is None:
                    metric_items = [
                        'mAP', 'mAP_50', 'mAP_75', 'mAP_s', 'mAP_m', 'mAP_l', 'LAMR'
                    ]

                # compute LAMR
                if 'LAMR' in metric_items:
                    lamr, mr_FPPI = self._compute_lamr(
                        cocoEval.evalImgs,
                        cocoEval.eval,
