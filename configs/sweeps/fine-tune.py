# model
model = dict(
    type='TwoStageDetector',  # use adaptive model
    ...
    roi_head=dict(
        type='StandardRoIHead',  # special head that returns more information
        bbox_roi_extractor=dict(type='SingleRoIExtractor',
                                roi_layer=dict(type='RoIAlign', output_size=7, sampling_ratio=0),
                                out_channels=256,
                                featmap_strides=[4, 8, 16, 32]),
        bbox_head=dict(
            type='Split2FCBBoxHead',
            in_channels=256,
...
# TRAINING
# fine-tuning: smaller lr, no lr updates, otherwise standard parameters
optimizer = dict(
    type='SGD',
    lr=0.001,
    momentum=0.9,
...
# run for 40 epochs and save best model, not last
