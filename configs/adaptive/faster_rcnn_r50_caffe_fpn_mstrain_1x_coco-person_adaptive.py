...
model = dict(
    ...
    train_cfg=dict(
        ...
        da=[
            ...
            #      lambd='coupled',
            #      transform='sample'),
            # dict(type='gpa', feat='feat_roi', loss_weights=dict(intra=10.0, inter=0.1), use_graph=True),
            # dict(type='gpa', feat='feat_rcnn_cls', loss_weights=dict(intra=100.0, inter=0.1)),
            dict(type='gpa',
                 feat='feat_rcnn_cls',
                 loss_weights=dict(intra=100.0, inter=0.1),
                 mode='ground_truth',
                 gt_iou_thrs=(.9, .1)),
            # dict(type='gpa', feat='feat_rcnn_bbox', loss_weights=dict(intra=100.0, inter=0.1), use_graph=True)
        ],
